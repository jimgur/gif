...
function download(options, stream, info) {
  ...
  // jimgur : check to see if we should abort the stream
  var totalSize = 0;
  var sizeLimit = (1<<20) * 4; // 4 MB
  stream.on('data', function(chunk){
    totalSize += chunk.length;
    if(totalSize > sizeLimit) {
      console.log('Stream size limit reached! Aborting the request!');
      stream.end();
      res.end();
      res.abort(); // abandon ship!!!
    }
  });
